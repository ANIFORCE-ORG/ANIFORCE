#!/usr/bin/env python3
# %%
"""调试 Runner.run_streamed 流式输出：ANIFORCE 场景。

运行：
  UV_CACHE_DIR=./uv_cache uv run python notebooks/04-runner/260701_02_runner_streaming_debug.py

验证点：
1. Runner.run_streamed(): 返回 RunResultStreaming
2. stream_events(): 实时收到 raw_response_event / run_item_stream_event
3. 有工具调用时，流里能看到 tool_called / tool_output / message_output_created
4. 流结束后，仍然可以读取 result.final_output 和 result.new_items
"""
...
async def run_streamed_demo() -> None:
    ...
    print("\n" + "=" * 80)
    print("流式事件：result.stream_events()")
    print("=" * 80 + "\n")
        ...
        if event.type == "raw_response_event":
            ...
            if raw_type == "response.created":
                ...
                if not in_text_stream:
                    in_text_stream = True
                    print("\n[raw text delta begin]")
                print(f"[delta] {getattr(event.data, 'delta', '')!r}", flush=True)

            elif raw_type == "response.completed":
                if in_text_stream:
                    print("\n[raw text delta end]")
                    in_text_stream = False
                print("[raw] response.completed")

        elif event.type == "run_item_stream_event":
            if in_text_stream:
                print("\n[raw text delta paused]")
                in_text_stream = False
            print(f"\n[run_item] name={event.name}")
            print(summarize_run_item(event.item))
            print("-" * 80)

        elif event.type == "agent_updated_stream_event":
            print(f"\n[agent_updated] new_agent={event.new_agent.name}")
            print("-" * 80)

        else:
            print(f"[unknown_event] {event}")
