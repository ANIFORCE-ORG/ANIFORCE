...
<template>
  <header class="flex items-center justify-between px-6 py-3 md:px-12 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-background-dark/80 backdrop-blur-md sticky top-0 z-50">
    ...
    <div class="flex items-center gap-3 shrink-0">
      ...
      <div class="relative">
        <button
          class="flex h-9 w-9 items-center justify-center rounded-full border-2 transition-all duration-200 cursor-pointer"
          :class="auth.isLoggedIn
            ? 'bg-primary/10 border-primary/30 hover:border-primary'
            : 'bg-slate-200 dark:bg-slate-700 border-white dark:border-slate-800 hover:border-primary'"
          @click="handleUserClick"
        >
          <span v-if="auth.isLoggedIn" class="text-xs font-bold text-primary">{{ auth.user?.name?.charAt(0) }}</span>
          <span v-else class="material-symbols-outlined text-lg text-slate-500">person</span>
        </button>
        ...
        <Transition name="fade">
          <div
            ...
            class="absolute right-0 top-full mt-2 w-48 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-xl py-2 z-50"
          >
