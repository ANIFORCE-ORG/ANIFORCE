  ...
  <div class="flex h-[calc(100vh-100px)] w-full overflow-hidden bg-slate-50 dark:bg-slate-950">
    ...
    <main class="flex-1 flex flex-col bg-white dark:bg-slate-900 overflow-hidden">
      <div class="border-b border-slate-200 dark:border-slate-800 px-[19px] py-[12px]">
        <h1 class="text-[15px] font-bold text-slate-900 dark:text-white">系统管理</h1>
        <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1">管理系统配置和全局用户账号</p>
      </div>

      <div class="flex-1 overflow-y-auto p-[19px]">
        <!-- 卡片式管理入口 -->
        <div v-if="!showSystemPanel && !showUserManagementPanel" class="grid gap-[12px] md:grid-cols-3">
          <section
