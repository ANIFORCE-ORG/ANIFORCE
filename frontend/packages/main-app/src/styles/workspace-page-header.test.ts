...
describe('Dashboard workspace page contract', () => {
  ...
  it('uses the compact Dashboard chart geometry and marker weights', () => {
    ...
    expect(dashboard).not.toContain('class="trend-metric-pills"')
    expect(dashboard).not.toContain('selectedTrendChartMetrics')
    expect(dashboard).not.toContain('toggleTrendMetric')
    expect(dashboard).toContain('<MetricSelector :model-value="trendMetricColumns"')
    expect(dashboard).toContain('<div class="chart-legend"><span v-for="metric in trendMetricColumns"')
