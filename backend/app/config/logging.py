"""Production-oriented structured logging configuration."""
...
from app.config.log_context import enrich_record
...
class InterceptHandler(logging.Handler):
    """Route stdlib/uvicorn/SQLAlchemy logging through Loguru."""
...
def setup_logging(
    ...
    *,
    ...
    """Configure one canonical sink per process plus stdlib interception."""
    if json_logs is None:
        json_logs = os.getenv("LOG_FORMAT", "text").lower() == "json"
    if console is None:
        console = os.getenv("LOG_OUTPUT", "console").lower() in {"console", "both"}
    ...
    # Normal SQL and HTTP client traffic is not application telemetry.
